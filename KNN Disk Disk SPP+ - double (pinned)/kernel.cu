...
int main(int argc, char* argv[])
{
	...
	unsigned int queryloops = querypoints / querypartitionsize;
	...
	// Query points Device Vector, copies data from Host Vector
	std::unique_ptr<thrust::device_vector<point_struct>[]> d_query_data(new thrust::device_vector<point_struct>[NUM_STREAMS]);
	...
	std::unique_ptr<KernelArray<point_struct>[]> k_d_query_data(new KernelArray<point_struct>[NUM_STREAMS]);
	...
	for (unsigned int i = 0; i < k * querypoints; i++) {
		query_distance_list.push_back({ 0,-1 });
	...
	std::unique_ptr<cudaStream_t[]> streams(new cudaStream_t[NUM_STREAMS]);
